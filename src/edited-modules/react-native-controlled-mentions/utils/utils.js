...
exports.replaceMentionValues =
  exports.getValueFromParts =
  exports.parseValue =
  exports.getMentionValue =
  exports.generateMentionPart =
  exports.generatePlainTextPart =
  exports.generateValueWithAddedSuggestion =
  exports.generateValueFromPartsAndChangedText =
  exports.getMentionPartSuggestionKeywords =
...
/**
 * RegEx grouped results. Example - "@[Full username](123abc)"
 * We have 4 groups here:
 * - The whole original string - "@[Full username](123abc)"
 * - Mention trigger - "@"
...
const defaultPlainStringGenerator = ({ trigger }, { username }) => `${trigger}${username}`;
 ...
 *
 * Examples where @username is just plain text yet, not mention:
 * '|abc @username dfg' - keyword is undefined
 * 'abc @| dfg' - keyword is ''
 * 'abc @username| dfg' - keyword is 'username'
 * 'abc @na|me dfg' - keyword is 'na'
 * 'abc @|username dfg' - keyword is against ''
 * 'abc @username |dfg' - keyword is 'username '
 * 'abc @username dfg|' - keyword is 'username dfg'
 * 'abc @username dfg |' - keyword is undefined (we have more than one space)
 * 'abc @username dfg he|' - keyword is undefined (we have more than one space)
 */
...
const getMentionValue = (trigger, suggestion) =>
  `${trigger}[${suggestion.username}](${suggestion.id})`;
exports.getMentionValue = getMentionValue;
const getMentionDataFromRegExMatchResult = ([, original, trigger, username, id]) => ({
  original,
  trigger,
  username,
  id,
...
const parseValue = (value, partTypes, positionOffset = 0) => {
  ...
  if (partTypes.length === 0) {
    ...
    const matches = Array.from(
      string_prototype_matchall_1.default(value !== null && value !== void 0 ? value : '', regex),
    );
...
const replaceMentionValues = (value, replacer) =>
  value.replace(mentionRegEx, (fullMatch, original, trigger, username, id) =>
    replacer({
      original,
      trigger,
      username,
